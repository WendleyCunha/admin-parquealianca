...
# Função para PDF S-21 (Mensal) - AJUSTADA CONFORME MODELO S-21
def gerar_pdf_registro_s21(row, mes_sel):
    ...
    # Lógica do Formulário S-21: Participou (Sim/Não) e Horas
    # No S-21, se houve qualquer atividade, marca-se "Participou"
    header = ["Participou no\nministério", "Estudos\nbíblicos", "Pioneiro\nauxiliar", "Horas", "Observações"]
    
    # Marcamos X se horas > 0 OU se houve estudos bíblicos
    check_min = "X" if row['horas'] > 0 or row['estudos_biblicos'] > 0 else ""
    check_pion = "X" if row['cat_oficial'] == "PIONEIRO AUXILIAR" else ""
    
    # Corpo da tabela com as horas separadas
